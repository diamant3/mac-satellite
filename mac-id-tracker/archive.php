...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($card)) {
        $cardErr = "<span class='text-danger'>* You did not select any card.</span><br />";
    }
    ...
    if (empty($cardErr) && empty($criteriaErr) && empty($searchErr)) {
        try {
            $data = $conn->prepare("SELECT * FROM {$card} WHERE {$criteria} LIKE '%{$search}%'");
            $data->execute();
            $count = $data->rowCount();
            $fetch = $data->fetchAll();
            ...
            <li class="nav-item active">
                ...
                <div id="card" class="collapse" aria-labelledby="headingPages" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item active" href="#">Archive</a>
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 text-gray-800">Cards / Archive Card Information</h1>
                    </div>
                        ...
                        <div class="col-lg-5">
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Search Card Data</h6>
                                </div>
                                <div class="card-body">
                                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                        ...
                                        <div class="input-group justify-content-around col-lg-12 col-md-12 col-sm-12 col-12">
                                            <?php echo $cardErr; ?>
                                            <select name="card" class="btn btn-primary btn-block shadow m-1">
                                            ...
                                            <select name="criteria" class="btn btn-primary btn-block shadow m-1">
                                                ...
                                                <option class="bg-white text-dark" value="id_number">ID Number</option>
                                                <option class="bg-white text-dark" value="firstname">First Name</option>
                            ...
                            <div class="card shadow mb-4">
                                ...
                                <div id="table_result" class="card-body">
                                    ...
                                    if ($count > 0) {
                                        ...
                                        <thead>
                                            <th class='text-dark'>Archive Status</th>
                                            <th class='text-dark'>ID Number</th>
                                            <th class='text-dark'>First Name</th>
                                            <th class='text-dark'>Last Name</th>
                                            <th class='text-dark'>Address</th>
                                            <th class='text-dark'>Card Type</th>
                                            <th class='text-dark'>Date Issued</th>
                                        ...
                                        foreach ($fetch as $row) {
                                            ...
                                            if ($status == 1) {
                                                echo "<td><button class='btn btn-secondary archive-action' data-id='{$row['id']}' data-cardtype='{$row['cardtype']}'>
                                                    <span id='archive-status'>Archived</span>
                                                    </button></td>";
                                            } else {
                                                echo "<td><button class='btn btn-warning archive-action' data-id='{$row['id']}' data-cardtype='{$row['cardtype']}'>
                                                    <span id='archive-status'>Not archive</span>
                                                    </button></td>";
                                            }
                                            echo "<td class='text-dark'>{$row['id_number']}</td>";
                                            echo "<td class='text-dark'>{$row['firstname']}</td>";
                                            echo "<td class='text-dark'>{$row['lastname']}</td>";
                                            echo "<td class='text-dark'>{$row['address']}</td>";
                                            echo "<td class='text-dark'>{$row['cardtype']}</td>";
                                            echo "<td class='text-dark'>{$row['issue_date']}</td>";
                                            if ($card === "white_id") {
                                                echo "<td class='text-dark'>No expiration Date</td>";
                                            } else {
                                                echo "<td class='text-dark'>{$row['exp_date']}</td>";
                                            }
                                            echo "</tr>";
                                        }
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.archive-action').click(function() {
            var id = $(this).data("id");
            var card = $(this).data("cardtype");
            var btn = $('.archive-action');

            $.ajax({
            url: 'update_archive.php',
            type: 'POST',
            ...
            success: function(response) {   
                if (response == 1) {
                    ...
                    alert("Archive Succesfully!");
                }else {
                    btn.removeClass("btn-warning");
                    btn.addClass("btn-secondary");
                    alert("Unarchive Succesfully!");
                }
