...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($cardErr) && empty($criteriaErr) && empty($searchErr)) {
        // for update-form.php session
        $_SESSION['card_db'] = $card;

        try {
            $data = $conn->prepare("SELECT * FROM {$card} WHERE {$criteria} LIKE '%{$search}%' AND is_archive = false OR is_archive IS NULL");
            $data->execute();
            $count = $data->rowCount();
            $fetch = $data->fetchAll();
                                ...
                                <div class="card-body">
                                    ...
                                    <?php echo $searchErr; ?>
                                        <div class="input-group mb-3 col-sm-12 col-md-12 col-lg-12">
                                            <input type="text" class="form-control bg-light" name="search" id="search" placeholder="Search" aria-label="search" aria-describedby="basic-addon2">
                                ...
                                <div id="table_result" class="card-body">
                                    ...
                                    if ($count > 0) {
                                        ...
                                        foreach ($fetch as $row) {
                                            ...
                                            echo "<td>";
                                            echo "<a href=\"update-form.php?id={$row['id']}\" class='btn btn-warning' title='Update Record' data-toggle=\"tooltip\">Update</a>";
                                            echo "</td>";
                                            echo "</tr>";
                                        }
