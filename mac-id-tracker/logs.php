...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($criteriaErr) && empty($searchErr)) {
        try {
            $data = $conn->prepare("SELECT * FROM history_log WHERE {$criteria} LIKE '%{$search}%' ORDER BY id DESC");
            $data->execute();
            $count = $data->rowCount();
            $fetch = $data->fetchAll();
...
<head>
    ...
    <title>MAC ID Tracker - Transaction Logs</title>
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 text-gray-800">Transaction Logs</h1>
                    </div>
                    ...
                    <div class="row">
                                    ...
                                    <?php echo $searchErr; ?>
                                        ...
                                        <div class="input-group justify-content-around col-lg-12 col-md-12 col-sm-12 col-12">
                                            ...
                                            <select name="criteria" class="btn btn-primary btn-block shadow m-1">
                                                ...
                                                <option class="bg-white text-dark" value="timestamp">Date/Time</option>
                                                <option class="bg-white text-dark" value="action">Action</option>
                                                <option class="bg-white text-dark" value="event">Event</option>
                                            </select>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-lg-7">
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Output Log</h6>
                                </div>
                                <div id="table_result" class="card-body">
                                    ...
                                    if ($count > 0) {
                                        ...
                                        <thead>
                                            <th class='text-dark'>In Charge</th>
                                            <th class='text-dark'>Action</th>
                                            <th class='text-dark'>Data Item</th>
                                        ...
                                        foreach ($fetch as $row) {
                                            echo "<tr>";
                                            echo "<td class='text-dark'>{$row['fullname']}</td>";
                                            echo "<td class='text-dark'>{$row['action']}</td>";
                                            echo "<td class='text-dark'>{$row['event']}</td>";
                                            echo "<td class='text-dark'>{$row['timestamp']}</td>";
                                            echo "</tr>";
                                        }
